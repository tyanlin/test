		...
		**/
		...
		for(int i = 0; i < groups.size(); i++) {
			...
			for(int j = 0; j < groups.get(i).size(); j++) {
				System.out.println("\tPoint: ID: "+groups.get(i).get(j).getID() + ",  id = " + groups.get(i).get(j).getV()[144]);
			}
		}
...
/*		System.out.println("------------------");
	...
	public Hashtable<String, Double> calculateDistance(ArrayList<Point> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			for (int j = 0; j < list.size(); j++) {
				if(u.getDis(list.get(i), list.get(j)) > THRESHOLD )
					continue;
				else
					disTable.put(i+"to"+j, u.getDis(list.get(i), list.get(j)));
			}
		}
		return disTable;
	}
	...
	public boolean inGroup(ArrayList<Point> list, Point p, Hashtable<String, Double> disTable, double e) {
		for (int i = 0; i < list.size(); i++) {
			...
			if(disTable.get(list.get(i).getID()+"to"+p.getID()) == null) 
				continue;
